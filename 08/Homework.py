...
def copy_contact():
    with open('phonebook.txt', 'r', encoding='UTF-8') as file, open('copyphonebook.txt', 'w', encoding='UTF-8') as coppy:
       contacts_list=file.read().rstrip().split("\n\n")
       for nn,contact in enumerate(contacts_list,1):
           print(nn,contact)
       copy_index = int(input('Какую строку вы хотите скопировать:  '))
       coppy.write(contacts_list[copy_index-1])
